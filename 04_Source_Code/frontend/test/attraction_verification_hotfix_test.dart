...
void main() {
  group('HOTFIX-RELEASE-001-H2: Frontend Attraction Verification & Badge Policy Tests', () {
    final testUser = User(
    ...
    test('1. ProductionConfig central defaults are valid', () {
      ...
      expect(ProductionConfig.maxAllowedLocationAccuracyMeters, equals(500.0));
    });
    ...
    test('3. Attraction place uses ATTRACTION_LOCATION verification type', () {
      expect(attractionGPS.reviewVerificationType, equals('ATTRACTION_LOCATION'));
    ...
    test('4. Missing coordinates attraction enables manual visit date fallback', () {
    ...
    test('5. Future visit date validation rejects future dates', () {
      ...
      bool isFutureValid = futureDate.isBefore(now) || futureDate.isAtSameMomentAs(now);
    ...
    test('6. Visit date older than 90 days rejects invalid dates', () {
      final now = DateTime.now();
      final oldestAllowed = now.subtract(Duration(days: ProductionConfig.defaultMaxVisitDateDaysAgo));
    ...
    testWidgets('7. Review badges correctly format GPS vs Visit Date vs Business QR badges', (WidgetTester tester) async {
    ...
    testWidgets('8. Bottom action bar uses SafeArea to prevent clipping', (WidgetTester tester) async {
      ...
      expect(find.byKey(const Key('bottom_action_container')), findsOneWidget);
    });

    // 9. Review ownership & actions retention (canEdit / canDelete)
    test('9. Review ownership flags properly compute edit/delete permissions', () {
      final review = Review(
        id: 'rev_owner_check',
        userId: testUser.id,
        storeId: attractionGPS.id,
    ...
    test('10. Place model falls back to BUSINESS_QR when verification type is unspecified', () {
    ...
    test('11. BUSINESS_QR stores are strictly excluded from attraction coordinate audit', () {
      final allPlaces = [businessStore, attractionGPS, attractionNoCoords];

      // Filter strictly for ATTRACTION_LOCATION places
      final attractionPlaces = allPlaces.where((p) => p.reviewVerificationType == 'ATTRACTION_LOCATION').toList();

      expect(attractionPlaces.contains(businessStore), isFalse);
      expect(attractionPlaces.length, equals(2));

      final missingCoordsAttractions = attractionPlaces.where((p) => p.latitude == null || p.longitude == null).toList();
      expect(missingCoordsAttractions.length, equals(1)); // only attractionNoCoords
