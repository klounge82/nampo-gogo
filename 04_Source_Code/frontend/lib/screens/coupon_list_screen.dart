...
class _CouponListScreenState extends State<CouponListScreen> {
  ...
  Future<void> _performExchange(Coupon coupon) async {
    ...
    try {
      final res = await _couponRepository.exchangeCoupon(coupon.id, userId: userId);
  ...
  void _showExchangeSuccessDialog(Coupon coupon) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('🎉 교환 성공!', style: TextStyle(fontWeight: FontWeight.bold), textAlign: TextAlign.center),
        content: Column(
          ...
          children: [
            ...
            const Text('쿠폰 교환이 정상 완료되었습니다.\n내 쿠폰함에서 바코드를 확인해 주세요.', textAlign: TextAlign.center, style: TextStyle(fontSize: 12.0)),
          ],
        ),
        actions: [
          Center(
          ...
          children: [
            Row(
              children: [
                ...
                Text(
                  '${coupon.costPoints} P',
                  style: const TextStyle(fontSize: 20.0, fontWeight: FontWeight.bold, color: AppColors.secondary),
                ),
              ],
            ),
            const SizedBox(height: 12.0),
            Text(
              coupon.description,
              style: const TextStyle(fontSize: 13.0, color: AppColors.textSecondary, height: 1.4),
            ),
            const SizedBox(height: 16.0),
            Text(
              '유효 기간: 발급 후 ${coupon.expiryDays}일 간 사용 가능',
              style: const TextStyle(fontSize: 12.0, color: AppColors.primary, fontWeight: FontWeight.w500),
            ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('포인트 교환소', style: TextStyle(fontWeight: FontWeight.bold)),
        backgroundColor: AppColors.surface,
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 16.0),
                      ElevatedButton(onPressed: _loadShopCoupons, child: const Text('다시 시도')),
                    ],
                  ),
                )
              : Column(
                  children: [
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          const Text('보유 중인 포인트', style: TextStyle(fontSize: 14.0, fontWeight: FontWeight.w500, color: AppColors.textSecondary)),
                          Text(
                            '$userPoints P',
                            style: const TextStyle(fontSize: 18.0, fontWeight: FontWeight.bold, color: AppColors.secondary),
                          ),
                        ],
                      ),
                    ),
                    ...
                    children: [
                      Text(
                        ...
                        style: TextStyle(
                          ...
                          color: canExchange ? AppColors.secondary : Colors.grey
                        ),
                      ),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 6.0, vertical: 2.0),
                        decoration: BoxDecoration(
                          color: canExchange ? AppColors.primary.withAlpha(26) : Colors.grey.withAlpha(26),
                        ...
                        child: Text(
                          ...
                          style: TextStyle(
                            fontSize: 9.0, 
                            fontWeight: FontWeight.bold, 
                            color: canExchange ? AppColors.primary : Colors.grey
                          ),
                        ),
                      ),
                    ],
