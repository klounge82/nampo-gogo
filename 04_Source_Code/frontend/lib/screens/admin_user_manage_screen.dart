...
class _AdminUserManageScreenState extends State<AdminUserManageScreen> {
  ...
  Future<void> _toggleUserStatus(User user) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text(label, style: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.secondary)),
        content: Text('정말로 ${user.nickname}님을 $label 처리하시겠습니까?\n정지 시 해당 사용자는 로그인이 즉시 차단됩니다.'),
        actions: [
          TextButton(onPressed: () => Navigator.of(ctx).pop(false), child: const Text('취소')),
          TextButton(
            onPressed: () => Navigator.of(ctx).pop(true),
            child: Text(label, style: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.secondary)),
          ),
        ],
      ),
    );
    ...
    try {
      await _adminRepository.updateUserStatus(user.id, newStatus, adminId: adminId);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('회원 계정 관리', style: TextStyle(fontWeight: FontWeight.bold)),
            ...
            child: Row(
              children: [
                ...
                ElevatedButton(
                  ...
                  child: const Text('검색', style: TextStyle(color: Colors.white, fontSize: 13.0)),
                ),
              ],
            ),
          ),
          ...
          Expanded(
            child: _isLoading
                ...
                : _errorMessage != null
                    ...
                    : _users.isEmpty
                        ? const Center(child: Text('해당 유저를 찾을 수 없습니다.', style: TextStyle(color: AppColors.textSecondary)))
                        : ListView.builder(
      ...
      child: ListTile(
        ...
        title: Row(
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: isBlocked ? Colors.red.withAlpha(20) : Colors.green.withAlpha(20),
                borderRadius: BorderRadius.circular(4.0),
              ),
              child: Text(
                isBlocked ? '이용정지' : '활성',
                style: TextStyle(fontSize: 9.0, fontWeight: FontWeight.bold, color: isBlocked ? Colors.red : Colors.green),
              ),
            ),
          ],
        ),
        subtitle: Column(
          ...
          children: [
            ...
            const SizedBox(height: 2.0),
            Text('포인트: ${user.currentPoints} P  |  역할: ${user.role}', style: const TextStyle(fontSize: 11.0, color: AppColors.textHint)),
          ],
        ),
        trailing: TextButton(
