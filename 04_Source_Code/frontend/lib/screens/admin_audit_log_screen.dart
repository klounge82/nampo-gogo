...
class _AdminAuditLogScreenState extends State<AdminAuditLogScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('감사 로그 조회', style: TextStyle(fontWeight: FontWeight.bold)),
      ...
      body: _isLoading
          ...
          : _errorMessage != null
              ...
              : _logs.isEmpty
                  ? const Center(
                      child: Text('기록된 감사 로그가 없습니다.', style: TextStyle(color: AppColors.textSecondary)),
                    )
                  : RefreshIndicator(
  ...
  Widget _buildLogCard(AdminAuditLogModel log) {
    final dateStr = '${log.createdAt.year}.${log.createdAt.month.toString().padLeft(2, '0')}.${log.createdAt.day.toString().padLeft(2, '0')} ${log.createdAt.hour.toString().padLeft(2, '0')}:${log.createdAt.minute.toString().padLeft(2, '0')}';
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              ...
              style: const TextStyle(fontSize: 13.0, color: AppColors.textPrimary, height: 1.35),
            ),
