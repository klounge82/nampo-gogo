...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('관리자 시스템', style: TextStyle(fontWeight: FontWeight.bold)),
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      ElevatedButton(onPressed: _loadDashboardStats, child: const Text('다시 시도')),
                    ],
                  ),
                )
              : RefreshIndicator(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildMenuCard(
                          ...
                          color: Colors.orange,
                          onTap: () => Navigator.of(context).push(
                            MaterialPageRoute(builder: (_) => const AdminStoreManageScreen()),
                          ),
                        ),
                        _buildMenuCard(
                          title: '인증 미션 관리',
                          ...
                          color: Colors.purple,
                          onTap: () => Navigator.of(context).push(
                            MaterialPageRoute(builder: (_) => const AdminMissionManageScreen()),
                          ),
                        ),
                        _buildMenuCard(
                          title: '예약 거래 내역',
                          ...
                          color: Colors.teal,
                          onTap: () => Navigator.of(context).push(
                            MaterialPageRoute(builder: (_) => const AdminReservationManageScreen()),
                          ),
                        ),
                        _buildMenuCard(
                          title: '리뷰 노출 차단',
                          ...
                          color: Colors.red,
                          onTap: () => Navigator.of(context).push(
                            MaterialPageRoute(builder: (_) => const AdminReviewManageScreen()),
                          ),
                        ),
                        _buildMenuCard(
                          title: '감사 로그 조회',
  ...
  Widget _buildStatsGrid() {
    return GridView.count(
      ...
      children: [
        _buildStatCard('총 가입자', '${_stats?['total_users'] ?? 0}명', Icons.people, Colors.blue),
        _buildStatCard('협약 매장', '${_stats?['total_stores'] ?? 0}개', Icons.storefront, Colors.orange),
        _buildStatCard('등록 미션', '${_stats?['total_missions'] ?? 0}개', Icons.assignment, Colors.purple),
        _buildStatCard('활성 예약', '${_stats?['active_reservations'] ?? 0}건', Icons.notifications_active, Colors.teal),
      ],
    );
  }
  ...
  Widget _buildMenuCard({
    ...
    return Container(
      ...
      child: ListTile(
        ...
        title: Text(title, style: const TextStyle(fontSize: 14.0, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
        subtitle: Text(subtitle, style: const TextStyle(fontSize: 11.0, color: AppColors.textSecondary)),
